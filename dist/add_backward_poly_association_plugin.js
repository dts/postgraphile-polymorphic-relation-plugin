...
exports.addBackwardPolyAssociation = (builder, option) => {
    // First add an inflector for polymorphic backrelation type name
    builder.hook('inflection', inflection => (Object.assign(Object.assign({}, inflection), { backwardRelationByPolymorphic(table, polyConstraint, isUnique) {
    ...
    builder.hook('GraphQLObjectType:fields', (fields, build, context) => {
        const { extend, getTypeByName, pgGetGqlTypeByTypeIdAndModifier, pgIntrospectionResultsByKind: introspectionResultsByKind, pgSql: sql, getSafeAliasFromResolveInfo, getSafeAliasFromAlias, graphql: { GraphQLNonNull, GraphQLList }, inflection, pgQueryFromResolveData: queryFromResolveData, pgOmit: omit, 
        // describePgEntity,
        // mapFieldToPgTable,
        pgPolymorphicClassAndTargetModels, } = build;
      ...
      you need to use addModelTableMappingPlugin and definePolymorphicCustom before this`);
        ...
        const backwardPolyAssociation = pgPolymorphicClassAndTargetModels
            .filter(con => {
            ...
            const primaryConstraint = introspectionResultsByKind.constraint.find(attr => attr.classId === table.id && attr.type === 'p');
            ...
            const isForeignKeyUnique = introspectionResultsByKind.constraint.find((c) => {
                ...
                if (c.keyAttributeNums.length !== 2)
                    return false;
                // It must be an unique constraint
                if (c.type !== 'u')
                    return false;
                // the two attributes must be xx_type, xx_id
                if (!c.keyAttributes.find(a => a.name === sourceTableId))
                    return false;
                if (!c.keyAttributes.find(a => a.name === sourceTableType))
                    return false;
                return true;
            });
            const fieldName = inflection.backwardRelationByPolymorphic(foreignTable, currentPoly, isForeignKeyUnique);
            ...
            const foreignTableConnectionType = getTypeByName(inflection.connection(foreignTableType.name));
            if (!primaryAttribute || primaryAttribute.length < 1) {
                return memo;
            }
            return Object.assign(Object.assign({}, memo), { [fieldName]: fieldWithHooks(fieldName, (p) => {
                    const { getDataFromParsedResolveInfoFragment, addDataGenerator } = p;
                    addDataGenerator((parsedResolveInfoFragment) => {
                        return {
                            pgQuery: (queryBuilder) => {
                                queryBuilder.select(() => {
                                    const resolveData = getDataFromParsedResolveInfoFragment(parsedResolveInfoFragment, !isForeignKeyUnique && isConnection
                                    ...
                                    const query = queryFromResolveData(sql.identifier(foreignTable.namespace.name, foreignTable.name), foreignTableAlis, resolveData, queryOptions, (innerQueryBuilder) => {
                                        innerQueryBuilder.parentQueryBuilder = queryBuilder;
                                        innerQueryBuilder.where(sql.query `(${sql.fragment `${tableAlias}.${sql.identifier(primaryAttribute[0].name)} = ${sql.fragment `${foreignTableAlis}.${sql.identifier(sourceTableId)}`}`}) and (
                              ${sql.fragment `${foreignTableAlis}.${sql.identifier(sourceTableType)} = ${sql.value(modelName)}`})`);
                                            ...
                                            innerQueryBuilder.beforeLock('orderBy', () => {
                                                ...
                                                if (!innerQueryBuilder.isOrderUnique(false)) {
                                                    ...
                                                    if (foreignTable.primaryKeyConstraint) {
                                                        const fPrimaryKeys = foreignTable.primaryKeyConstraint.keyAttributes;
                                                        fPrimaryKeys.forEach((key) => {
                                                            innerQueryBuilder.orderBy(sql.fragment `${innerQueryBuilder.getTableAlias()}.
                                  ${sql.identifier(key.name)}`, true);
                                                        });
                                                    }
                                                }
...
//# sourceMappingURL=add_backward_poly_association_plugin.js.map